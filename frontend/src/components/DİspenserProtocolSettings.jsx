...
export default function DispenserProtocolSettings({ onNavigate, targetId }) {
  ...
  const [DispenserProtocols, setDispenserProtocols] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      let res1 = await triggerTableContent({ tableName: "DispenserProtocols" });
      if (res1) {
        const filtered = res1.filter(obje => obje.ID === targetId);
        setDispenserProtocols(filtered);
      }
    };
    fetchData();
  }, [targetId]);
  ...
  const handleInputChange = (index, field, value) => {
    const updatedData = [...DispenserProtocols];
    updatedData[index][field] = value;
    setDispenserProtocols(updatedData);
  };

  const handleSave = async (item) => {
    await triggerTableUpdate({ 
      tableName: "DispenserProtocols", 
      content: item 
    });
  };
  ...
  return (
    <div className="w-full mx-auto p-4 md:p-8 animate-in fade-in duration-700 min-h-screen bg-slate-950 text-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
        <div>
          <h2 className="text-3xl font-black text-white flex items-center gap-3">
            ...
            Dispenser Protocol Configuration
          </h2>
          <p className="text-slate-400 text-sm mt-1">Dispenser haberleşme protokollerini yapılandırın.</p>
        </div>
      </div>

      <div className="space-y-4">
        {loading2 && DispenserProtocols.length === 0 ? (
          ...
          DispenserProtocols.map((item, index) => (
            <div key={index} 
      ...
      {/* Empty State */}
      {DispenserProtocols.length === 0 && !loading2 && (
