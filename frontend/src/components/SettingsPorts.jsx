import { Save, Trash2, Cpu, Edit3 } from "lucide-react";
import { useTables } from "./../hooks/dbTransactions";
import { useEffect, useState } from 'react';

export default function SettingsPorts({onNavigate,targetId}) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            res1 = res1.filter(obje => obje.SCPNo === targetId || targetId == -1)
            if (res1) setSettingsPorts(res1);
        };
        fetchData();
    }, []);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div>
                    <h2 className="text-3xl font-black text-slate-900 flex items-center gap-3">
                        ...
                        Settings Ports Configuration                    </h2>
                </div>
            </div>
                        ...
                        <div key={index} className="bg-white border border-slate-200 rounded-2xl p-5 hover:border-emerald-400 transition-all shadow-sm">
                            <div className="flex items-center gap-8">
                                ...
                                <div className="min-w-[80px] border-r border-slate-100 pr-6">
                                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">PORT ID</div>
                                    <div className="text-xl font-black text-slate-800">#{item.PortID}</div>
                                </div>
                                ...
                                <div className="flex-grow flex flex-col gap-1.5 group/field">
                                    ...
                                    <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-4 focus-within:bg-white focus-within:ring-2 focus-within:ring-emerald-100 transition-all">
                                        <input
                                            type="text"
                                            value={item.Baudrate || ""}
                                            onChange={(e) => handleInputChange(index, 'Baudrate', e.target.value)}
                                            className="bg-transparent border-none w-full text-sm font-bold p-3 outline-none text-slate-700"
                                            placeholder=" "
                                        />
                                    </div>
                                ...
                                <div className="flex-grow flex flex-col gap-1.5 group/field">
                                    ...
                                    <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-4 focus-within:bg-white focus-within:ring-2 focus-within:ring-emerald-100 transition-all">
                                        <input
                                            type="text"
                                            value={item.Parity || ""}
                                            onChange={(e) => handleInputChange(index, 'Parity', e.target.value)}
                                            className="bg-transparent border-none w-full text-sm font-bold p-3 outline-none text-slate-700"
                                            placeholder=" "
                                        />
                                    </div>
                                ...
                                <div className="flex-grow flex flex-col gap-1.5 group/field">
                                    ...
                                    <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-4 focus-within:bg-white focus-within:ring-2 focus-within:ring-emerald-100 transition-all">
                                        <input
                                            type="text"
                                            value={item.StopBit || ""}
                                            onChange={(e) => handleInputChange(index, 'StopBit', e.target.value)}
                                            className="bg-transparent border-none w-full text-sm font-bold p-3 outline-none text-slate-700"
                                            placeholder=" "
                                        />
                                    </div>
                                ...
                                <div className="flex-grow flex flex-col gap-1.5 group/field">
                                    ...
                                    <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-4 focus-within:bg-white focus-within:ring-2 focus-within:ring-emerald-100 transition-all">
                                        <input
                                            type="text"
                                            value={item.DataBit || ""}
                                            onChange={(e) => handleInputChange(index, 'DataBit', e.target.value)}
                                            className="bg-transparent border-none w-full text-sm font-bold p-3 outline-none text-slate-700"
                                            placeholder=" "
                                        />
                                    </div>
                                ...
                                <div className="flex-grow flex flex-col gap-1.5 group/field">
                                    <div className="flex justify-between items-center px-1">
                                        <label className="text-[11px] font-bold text-slate-500 uppercase flex items-center gap-1">
                                            <Edit3 size={12} /> FlowControl
                                        </label>
                                    </div>
                                    <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-4 focus-within:bg-white focus-within:ring-2 focus-within:ring-emerald-100 transition-all">
                                        <input
                                            type="text"
                                            value={item.FlowControl || ""}
                                            onChange={(e) => handleInputChange(index, 'FlowControl', e.target.value)}
                                            className="bg-transparent border-none w-full text-sm font-bold p-3 outline-none text-slate-700"
                                            placeholder=" "
                                        />
                                    </div>
