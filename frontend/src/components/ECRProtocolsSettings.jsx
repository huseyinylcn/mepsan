...
export default function ECRProtocolsSettings() {
  const { triggerTableContent, loading2, triggerTableUpdate, loading3 } = useTables();
  const [ECRProtocols, setECRProtocols] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      const res1 = await triggerTableContent({ tableName: "ECRProtocols" });
      if (res1) setECRProtocols(res1);
