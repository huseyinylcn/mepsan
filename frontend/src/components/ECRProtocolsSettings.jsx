...
export default function ECRProtocolsSettings({ setActivePage, targetId }) {
  const { triggerTableContent, loading2, triggerTableUpdate, loading3 } = useTables();
  const [ECRProtocols, setECRProtocols] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      let res1 = await triggerTableContent({ tableName: "ECRProtocols" });
      if (res1) {
        // targetId == -1 ise tüm listeyi, değilse sadece ilgili ID'yi getirir
        const filtered = res1.filter(obj => obj.ID == targetId || targetId == -1);
        setECRProtocols(filtered);
      }
    };
    fetchData();
  }, [targetId]); // targetId değiştiğinde listeyi yenile

  const handleInputChange = (index, field, value) => {
    const updatedData = [...ECRProtocols];
    updatedData[index][field] = value;
    setECRProtocols(updatedData);
  };

  const handleSave = async (item) => {
    await triggerTableUpdate({ 
      tableName: "ECRProtocols", 
      content: item 
    });
  };
  ...
  return (
    <div className="w-full mx-auto p-4 md:p-8 animate-in fade-in duration-700 min-h-screen bg-slate-950 text-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
        <div>
          <h2 className="text-3xl font-black text-white flex items-center gap-3">
            ...
            ECR Protocol Configuration
          </h2>
          <p className="text-slate-400 text-sm mt-1">Ödeme kaydedici cihaz protokol tanımlamalarını yönetin.</p>
        </div>
      </div>

      {/* Liste Alanı */}
      <div className="space-y-4">
        {loading2 && ECRProtocols.length === 0 ? (
      ...
      {/* Empty State */}
      {ECRProtocols.length === 0 && !loading2 && (
        <div className="text-center py-20 bg-slate-900/50 rounded-3xl border-2 border-dashed border-slate-800 text-slate-500">
          ...
          <p className="font-bold uppercase tracking-widest text-xs">No protocols were found.</p>
        </div>
      )}
    </div>
  );
