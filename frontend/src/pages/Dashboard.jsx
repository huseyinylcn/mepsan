...
const items = [
  ...
  { 
    title: "Archive", 
    icon: Archive,
    description: "Günlük, haftalık ve aylık sistem istatistiklerini detaylı grafiklerle inceleyin." ,
    id:"archive" 
  ...
  { 
    title: "Log Settings", 
    icon: FileText,
    description: "View and modify the paths to log files." ,
    id:"logSettings",
...
export default function Dashboard({ setActivePage,targetId }) {
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-5 auto-rows-[160px]"> 
          {items.map((item, index) => {
            ...
            return (
              <button
                key={index}
                onClick={()=>{setActivePage(item.id); targetId(-1); }}
